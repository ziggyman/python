...
#        print('sumOfSpectra = ',sumOfSpectra)
        label = ''
        if negativeIndices is not None:
            with open('/Users/azuri/daten/uni/HKU/Pa30/Pa30_GTC_coadded_radial_velocity_corrected_'+('far' if i == 0 else 'near')+'_side.dat','w') as fOut:
                for iWLen in range(len(wLen)):
                    fOut.write('%10.5e %10.5e\n' % (wLen[iWLen], sumOfSpectra[iWLen]))
            if i == 0:
                label='far side'
            else:
                label='near side'
...
#        plt.show()
        label = ''
        with open('/Users/azuri/daten/uni/HKU/Pa30/Pa30_WIYN_coadded_radial_velocity_corrected_'+('far' if vrad[0] > 0 else 'near')+'_side.dat','w') as fOut:
            for iWLen in range(len(wLen)):
                fOut.write('%10.5e %10.5e\n' % (wLen[iWLen],sumOfSpectra[iWLen]))
        if vrad[0] > 0:
            label='far side'
        ...
        plt.plot(wLen,gauss2(wLen,popt[0],popt[1],popt[2],popt[3],popt[4],popt[5]),label='fit far side' if (vrad[0] > 0) else 'fit near side')
        ratio = area6716 / area6731
...
def fitSIIAll():
    with open('/Users/azuri/daten/uni/HKU/Pa30/Pa30_WIYN_coadded_radial_velocity_corrected_far_side.dat') as f:
        linesWIYNFar = f.readlines()
    with open('/Users/azuri/daten/uni/HKU/Pa30/Pa30_WIYN_coadded_radial_velocity_corrected_near_side.dat') as f:
        linesWIYNNear = f.readlines()
    with open('/Users/azuri/daten/uni/HKU/Pa30/Pa30_GTC_coadded_radial_velocity_corrected_far_side.dat') as f:
        linesGTCFar = f.readlines()
    with open('/Users/azuri/daten/uni/HKU/Pa30/Pa30_GTC_coadded_radial_velocity_corrected_near_side.dat') as f:
        linesGTCNear = f.readlines()

    for iLine in range(len(linesWIYNFar)):
        linesWIYNFar[iLine] = linesWIYNFar[iLine].strip().split(' ')
    print('linesWIYNFar = ',len(linesWIYNFar),': ',linesWIYNFar)
    for iLine in range(len(linesWIYNNear)):
        linesWIYNNear[iLine] = linesWIYNNear[iLine].strip().split(' ')
    print('linesWIYNNear = ',len(linesWIYNNear),': ',linesWIYNNear)
    for iLine in range(len(linesGTCFar)):
        linesGTCFar[iLine] = linesGTCFar[iLine].strip().split(' ')
    print('linesGTCFar = ',len(linesGTCFar),': ',linesGTCFar)
    for iLine in range(len(linesGTCNear)):
        linesGTCNear[iLine] = linesGTCNear[iLine].strip().split(' ')
    print('linesGTCNear = ',len(linesGTCNear),': ',linesGTCNear)

    wLen = np.array([float(x[0]) for x in linesWIYNFar])

    print('WIYN far side:')
    fluxWIYNFar = np.array([float(x[1]) for x in linesWIYNFar])
    area6716WIYNFar,area6731WIYNFar,poptWIYNFar = getAreas2Gauss(wLen,
                                                                 fluxWIYNFar,
                                                                 np.amax(fluxWIYNFar[:int(fluxWIYNFar.shape[0]/2)]),
                                                                 np.amax(fluxWIYNFar[int(fluxWIYNFar.shape[0]/2):]),
                                                                 mean1,
                                                                 mean2,
                                                                 sigma,
    ...
    ratio = area6716WIYNFar / area6731WIYNFar
    print('ratio of areas = ',ratio)
    den = S2.getTemDen(int_ratio=ratio,tem=10000.,wave1=6716,wave2=6731,maxIter=1000)
    print('electron density = ',den)
    print(' ')

    print('WIYN near side:')
    fluxWIYNNear = np.array([float(x[1]) for x in linesWIYNNear])
    area6716WIYNNear,area6731WIYNNear,poptWIYNNear = getAreas2Gauss(wLen,
                                                                 fluxWIYNNear,
                                                                 np.amax(fluxWIYNNear[:int(fluxWIYNNear.shape[0]/2)]),
                                                                 np.amax(fluxWIYNNear[int(fluxWIYNNear.shape[0]/2):]),
                                                                 mean1,
                                                                 mean2,
                                                                 sigma,
    ...
    ratio = area6716WIYNNear / area6731WIYNNear
    print('ratio of areas = ',ratio)
    den = S2.getTemDen(int_ratio=ratio,tem=10000.,wave1=6716,wave2=6731,maxIter=1000)
    print('electron density = ',den)
    print(' ')

    print('WIYN far and near combined:')
    fluxWIYNNearAndFar = np.array([float(x[1]) for x in linesWIYNNear]) + np.array([float(x[1]) for x in linesWIYNFar])
    area6716WIYNNearAndFar,area6731WIYNNearAndFar,poptWIYNNearAndFar = getAreas2Gauss(wLen,
                                                                 fluxWIYNNearAndFar,
                                                                 np.amax(fluxWIYNNearAndFar[:int(fluxWIYNNearAndFar.shape[0]/2)]),
                                                                 np.amax(fluxWIYNNearAndFar[int(fluxWIYNNearAndFar.shape[0]/2):]),
                                                                 mean1,
                                                                 mean2,
                                                                 sigma,
    ...
    ratio = area6716WIYNNearAndFar / area6731WIYNNearAndFar
    print('ratio of areas = ',ratio)
    den = S2.getTemDen(int_ratio=ratio,tem=10000.,wave1=6716,wave2=6731,maxIter=1000)
    print('electron density = ',den)
    print(' ')
    ...
    print('GTC far side:')
    fluxGTCFar = np.array([float(x[1]) for x in linesGTCFar])
    area6716GTCFar,area6731GTCFar,poptGTCFar = getAreas2Gauss(wLen,
                                                                 fluxGTCFar,
                                                                 np.amax(fluxGTCFar[:int(fluxGTCFar.shape[0]/2)]),
                                                                 np.amax(fluxGTCFar[int(fluxGTCFar.shape[0]/2):]),
                                                                 mean1,
                                                                 mean2,
                                                                 sigma,
    ...
    ratio = area6716GTCFar / area6731GTCFar
    print('ratio of areas = ',ratio)
    den = S2.getTemDen(int_ratio=ratio,tem=10000.,wave1=6716,wave2=6731,maxIter=1000)
    print('electron density = ',den)
    print(' ')
    ...
    print('GTC near side:')
    fluxGTCNear = np.array([float(x[1]) for x in linesGTCNear])
    area6716GTCNear,area6731GTCNear,poptGTCNear = getAreas2Gauss(wLen,
                                                                 fluxGTCNear,
                                                                 np.amax(fluxGTCNear[:int(fluxGTCNear.shape[0]/2)]),
                                                                 np.amax(fluxGTCNear[int(fluxGTCNear.shape[0]/2):]),
                                                                 mean1,
                                                                 mean2,
                                                                 sigma,
    ...
    ratio = area6716GTCNear / area6731GTCNear
    print('ratio of areas = ',ratio)
    den = S2.getTemDen(int_ratio=ratio,tem=10000.,wave1=6716,wave2=6731,maxIter=1000)
    print('electron density = ',den)
    print(' ')

    print('GTC far and near combined:')
    fluxGTCNearAndFar = np.array([float(x[1]) for x in linesGTCNear]) + np.array([float(x[1]) for x in linesGTCFar])
    area6716GTCNearAndFar,area6731GTCNearAndFar,poptGTCNearAndFar = getAreas2Gauss(wLen,
                                                                 fluxGTCNearAndFar,
                                                                 np.amax(fluxGTCNearAndFar[:int(fluxGTCNearAndFar.shape[0]/2)]),
                                                                 np.amax(fluxGTCNearAndFar[int(fluxGTCNearAndFar.shape[0]/2):]),
                                                                 mean1,
                                                                 mean2,
                                                                 sigma,
    ...
    ratio = area6716GTCNearAndFar / area6731GTCNearAndFar
    print('ratio of areas = ',ratio)
    den = S2.getTemDen(int_ratio=ratio,tem=10000.,wave1=6716,wave2=6731,maxIter=1000)
    print('electron density = ',den)
    print(' ')
