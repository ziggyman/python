...
#           ]
#idFiles = ['/Users/azuri/daten/uni/HKU/HASH/HASH_spectra/helen/bipolar.txt',
...
for idFile in idFiles:
    ...
    with open(idFile[:idFile.rfind('/')+1]+prefix+'_fetch.bat','w') as w:
        w.write('mkdir vanessa\n')
        w.write('mkdir vanessa/'+prefix+'\n')
        for id in ids:
            specNames = csvFiles.getData('fileName',csvFiles.find('idPNMain',id))
            print('specNames = ',specNames)
            w.write('mkdir vanessa/'+prefix+'/'+str(id)+'\n')
            for iSpec in range(len(specNames)):
                w.write('cp /data/mashtun/'+csvInfo.getData('path',csvInfo.find('name',csvFiles.getData('setname',csvFiles.find('fileName',specNames[iSpec])[0]))[0])+specNames[iSpec]+' vanessa/'+prefix+'/'+str(id)+'/\n')
