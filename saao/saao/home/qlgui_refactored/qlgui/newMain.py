...
from scipy.stats.mstats import mode as statmode
...
from obslogPane import ObsLogPane
...
#        QtCore.QObject.connect(self.ui.useSB1Check, QtCore.SIGNAL(_fromUtf8("toggled(bool)")), self.updateAll)
        QtCore.QObject.connect(self.ui.useSB1Check, QtCore.SIGNAL(_fromUtf8("toggled(bool)")), self.sideBandToggled)
        QtCore.QObject.connect(self.ui.useSB2Check, QtCore.SIGNAL(_fromUtf8("toggled(bool)")), self.sideBandToggled)
...
#        QtCore.QObject.connect(self.ui.raw2DGraph,QtCore.SIGNAL("MOUSE_MOVED"),self.display2Dlabel)
        QtCore.QObject.connect(self.ui.raw2DGraph,QtCore.SIGNAL(_fromUtf8("enterEvent()")),self.display2Dlabel)
...
#        self.logger.debug('**',self.sky1d)
    ...
    def loadPars(self):
        #settingsFile = '%s/qlsettings.json'%self.settingsdir
        settingsFile = '/home/ccd/.spup/qlsettings.json'
        print("Setting file = {}".format(settingsFile))
        if os.path.isfile(settingsFile):
            f = open(settingsFile,'r')
            pars = json.load(f)
            f.close()

            try: # we want to use tonight's datadir if it exists!
                f=open('/home/ccd/.spup/datadir.json','r')
...
#        self.updateAll()
        # check if the last used directory is the same as the datadir written by spupnic GUI
        # if so, let's re-load last frame used. If not, unset frame name, etc. (assume we have started on a new night):
#        self.ui.lineEditCurrFrame.setText(self.pars['currFrame'])
#        self.loadFrame()

    def applySettings(self):
        # apply settings from re-loaded pars file to selectable options, like skysub?, side-band?, etc.
        #self.ui.checkBoxdoSkySub.setCheckState(self.pars['SkySub'])
        self.ui.lineEditDataDir.setText(self.pars['dataDir'])
        self.ui.checkBoxShowSB.setCheckState(self.pars['showSideBands'])
        #self.ui.checkBoxShowExtractWin.setCheckState(self.pars[''])
        #these seem to be overridden when loading new frame anyway. leave for now:

        self.ui.extractWinSlider.setLowerValue(self.pars['extractWin_x0'])
        self.ui.extractWinSlider.setUpperValue(self.pars['extractWin_x1'])
        self.extractWin_x0 = self.pars['extractWin_x0']
        self.extractWin_x1 = self.pars['extractWin_x1']

        self.ui.SB1Slider.setLowerValue(self.pars['SB1_x0'])
        self.ui.SB1Slider.setUpperValue(self.pars['SB1_x1'])
        self.ui.SB2Slider.setLowerValue(self.pars['SB2_x0'])
        self.ui.SB2Slider.setUpperValue(self.pars['SB2_x1'])
        #*** should really get rid of these duplicate variables elsewhere in code and just use self.pars['*']
        self.SB1_x0 = self.pars['SB1_x0']
        self.SB1_x1 = self.pars['SB1_x1']
        self.SB2_x0 = self.pars['SB2_x0']
        self.SB2_x1 = self.pars['SB2_x1']


        # let's concentrate on remembering things like user's preferred extraction method.
        self.ui.comboBoxExtractionType.setCurrentIndex(int(self.pars['extrType']))
        self.ui.useSB1Check.setCheckState(self.pars['useSB1'])
        self.ui.useSB2Check.setCheckState(self.pars['useSB2'])
...
#            print tgrang
    ...
    def showRefArc(self):
        ...
        try:
            ...
            # try to set to reasonable scale:
            ok = np.where(y > 1000.)[0]
            ymode = statmode(y[ok])[0][0]
            y = y-ymode

            if(len(x)==2048):
                # trim:
                # get regions from 2D where it is set to nan:
                check2D = np.median(self.data2D,0)
                bad = np.where(check2D==np.nan)[0]
...
#        refArcFile=libdir+'ref_%s_%s.txt'%(self.grating,self.lamp)
        self.findNearestRefArc()
        refArcFile = np.copy(self.reffile)
...
#        print '**',lines
        ...
        try:
            # mask all spec to be same lengths:
            check2D = np.median(self.data2D,0)
#            print check2D
            bad = np.where(check2D==np.nan)[0]
#            print check2D.shape
            good = np.where(np.isfinite(check2D))[0]
#            print len(good)
            arcspec=np.copy(arcspec[good])
            reflam=np.copy(reflam[good])
            refspec=np.copy(refspec[good])
        except:
            print 'cannot mask bad regions at ends of spec. skipping!'
            pass

#        print len(arcspec),len(reflam),len(refspec)
#        stop()
        try:
            # mute output:
...
##                lamcal = wlc_arc(arcspec,reflam,refspec,linelist=linelist,maxCoarseShift=500.,maxFineShift=10.,iplot=False,idebug=False)
                ...
                lamcal = wlc_arc(arcspec,reflam,refspec,linelist=linelist,maxCoarseShift=200.,maxFineShift=10.,iplot=False,idebug=False)
...
#        if(self.imagetype=='arc'): print self.lamp
    ...
    def insertNaNs(self):
        # set useless regions of image to NaNs to aid plotting:
        self.data2D[:,0:98] = np.nan
        self.data2D[:,2145:] = np.nan
...
#        if(self.imagetype<>'object' ): self.data2DSkySub=self.data2D
    ...
    def updatezScaling(self):
        if(str(self.ui.comboBoxScaling.currentText())=='ZScale'): 
            clean2Ddata = np.nan_to_num(self.data2D)
            self.z0,self.z1=zscale(clean2Ddata)
        if(str(self.ui.comboBoxScaling.currentText())=='Min/Max'): 
            self.z0=np.nanmin(self.data2D)
            self.z1=np.nanmax(self.data2D)
...
#            self.skysub2DView.canvas.clear()
        ...
        f = open('%s/qlsettings.json'%self.settingsdir,'w')
        json.dump(self.pars,f)
        f.close()
